...
const files = ROOTS.filter((r) => fs.existsSync(path.join(root, r))).flatMap((r) => walk(path.join(root, r)));
...
for (const f of files) sig.set(path.relative(root, f), fs.readFileSync(f, "utf8").split(/\r?\n/).map(norm));
...
for (const [key, places] of blocks) {
  ...
  for (const p of places) if (!unique.has(p.file + ":" + p.start)) unique.set(p.file + ":" + p.start, p);
  const list = [...unique.values()];
  if (list.length < 2) continue;
  const filesSet = new Set(list.map((p) => p.file));
  found.push({ lines: MIN, files: filesSet.size, places: list, preview: key.split("\n").slice(0, 3).join(" | ").slice(0, 110) });
}
...
for (const b of found) {
  const key = b.places.map((p) => p.file + ":" + p.start).sort().join(",");
  if (dropped.has(key)) continue;
  result.push(b);
  // помечаем все под-блоки того же набора мест как вложенные
  for (let s = 1; s < MIN; s++) {
    for (const p of b.places) dropped.add(b.places.map((x) => x.file + ":" + (x.start + s)).sort().join(","));
  }
}

console.log(`Файлов просканировано: ${files.length}; минимальный блок: ${MIN} строк; найдено дублей: ${result.length}`);
