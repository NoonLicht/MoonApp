...
# Знаки ударения XTTS не понимает: в его словаре и комбинирующая акута (U+0301),
# и «+» — неизвестные символы. Проверено: «замо́к» → [267, 3428, 3447, 1, 3384],
# где 1 — <unk> ВНУТРИ слова, и это слышно как щелчок/сбой. Ударения нужны
# русскому F5-TTS («+» перед гласной: так их ставит RUAccent, см.
# server/engines/ru_accent.py), а для XTTS их вырезаем.
_COMBINING = re.compile("[\u0300-\u036f\u0483-\u0489]")
_PLUS_BEFORE_VOWEL = re.compile(r"\+([аеёиоуыэюяАЕЁИОУЫЭЮЯ])")


def drop_stress(text):
    """Убрать знаки ударения (акуту и «+» перед гласной) из текста.

    «+» убирается только перед гласной: плюс в остальных местах — обычный
    символ текста («C++», «2 + 2»), и трогать его нельзя.
    """
    return _PLUS_BEFORE_VOWEL.sub(r"\1", _COMBINING.sub("", str(text or "")))
