...
let PS = { enabled: false, port: DEF_PORT, running: false, child: null, pingMs: null, country: null, error: "" };
...
function parseVlessLink(link) {
  try {
    ...
    const p = {};
    if (qs) for (const pair of qs.split("&")) { const [k, v] = pair.split("="); if (k) p[decodeURIComponent(k)] = v ? decodeURIComponent(v) : ""; }
...
function jsonNodeToParts(o) {
  ...
  const port = o.port !== undefined ? Number(o.port) : o.server_port !== undefined ? Number(o.server_port) : NaN;
...
function inputIsValid(profile) { return !!parseProfile(profile); }
...
function ensureSocks(cfg, port) {
  ...
  if (!cfg.route) cfg.route = { auto_detect_interface: true, rules: [{ outbound: "direct", ip_is_private: true }] };
  return cfg;
}
...
function buildSBConfig({ uuid, server, port, params, proxyPort }) {
  ...
  const allowedFlow = (rawFlow === "xtls-rprx-vision")
    ? rawFlow
    : (rawFlow.startsWith("xtls-rprx-") ? "" : rawFlow);
  // Если flow xtls, а security не задан — TLS включается по умолчанию
  const needsTls = params.security === "tls" || params.security === "reality" || rawFlow.startsWith("xtls-");
  const ob = { type: "vless", tag: "proxy", server, server_port: port, uuid, flow: allowedFlow, packet_encoding: "xudp" };
  if (needsTls) {
    const tls = { enabled: true };
    if (params.fp) tls.utls = { enabled: true, fingerprint: params.fp || "chrome" };
    if (params.security === "reality" || params.pbk) tls.reality = { enabled: true, public_key: params.pbk || "", short_id: params.sid || "" };
...
function normalizeConfig(cfg) {
  ...
  if (!cfg.dns && cfg.experimental && typeof cfg.experimental === "object" && cfg.experimental.dns) {
...
function runSBVer(b) {
  return new Promise((r) => {
    ...
    execFile(b, ["version"], { timeout: 8000, windowsHide: true, maxBuffer: 256 * 1024 }, (e, o) => {
...
function saveCfg(p) { const cur = settings.get("proxy") || {}; settings.set({ proxy: { ...cur, ...p } }); }
// --- Запуск / остановка ---
...
async function startProxy(profileArg) {
  ...
  if (!prof) { PS = { ...PS, enabled: false, running: false, error: "Invalid profile (link or JSON)" }; return getStatus(); }
  ...
  try { const olds = fs.readdirSync(cDir).filter(f => f.endsWith(".json")); for (const f of olds) fs.unlinkSync(path.join(cDir, f)); } catch {}
  ...
  try { fs.writeFileSync(cPath, JSON.stringify(cfg, null, 2), "utf8"); } catch (e) { PS = { ...PS, error: "Config write failed" }; return getStatus(); }
  return new Promise((r) => {
    const child = spawn(bin.path, ["run", "-c", cPath, "--disable-color"], { windowsHide: true, stdio: ["ignore", "pipe", "pipe"] });
    PS.child = child; PS.port = port; PS.running = false; PS.enabled = false; PS.error = "";
    let bo = "";
    const bt = setTimeout(() => { if (!PS.running && !PS.error) { PS.running = true; PS.enabled = true; logger.info("proxy.started", { port, pid: child.pid }); r(getStatus()); } }, 3000);
child.stdout.on("data", () => {});
    child.stderr.on("data", (d) => { bo += d.toString(); if (bo.includes("start") || bo.includes("inbound") || bo.includes("socks")) { PS.running = true; PS.enabled = true; clearTimeout(bt); r(getStatus()); } });
    child.on("error", (e) => { clearTimeout(bt); PS = { ...PS, running: false, enabled: false, error: e.message }; r(getStatus()); });
    child.on("close", (code) => {
      clearTimeout(bt);
      if (PS.running) { PS.running = false; PS.enabled = false; PS.child = null; }
      else { PS.error = bo.slice(-500) || `sing-box exited with code ${code}`; if (!PS.enabled) r(getStatus()); }
    });
  });
}
...
async function pingProxy(timeout = 8000) {
  if (!PS.running || !PS.enabled) { PS.pingMs = null; PS.country = null; return { pingMs: null, country: null, error: "Not running" }; }
  ...
  for (const svc of PING_SERVICES) {
    ...
    try {
      const res = await socksFetch(svc.url, { signal: ctrl.signal, headers: { "User-Agent": "Mozilla/5.0" } });
      clearTimeout(timer);
  ...
  PS.pingMs = null; PS.country = null;
  const friendly = /hang up|ECONNRESET|socket/i.test(lastErr) ? "Connection failed (proxy unreachable)" : (lastErr || "Ping failed");
...
async function installSB() {
  ...
  try {
    is.phase = "download";
    const res = await fetch(SB_URL, { redirect: "follow", headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" } });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    ...
    ws.on("error", () => {});
    for await (const chunk of res.body) { const b = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk); rcvd += b.length; is.progress = declared ? Math.min(100, Math.round(100 * rcvd / declared)) : 0; if (!ws.write(b)) await new Promise((r) => ws.once("drain", r)); }
    await new Promise((resolve, reject) => ws.end((err) => (err ? reject(err) : resolve())));
    is.phase = "extract"; is.progress = 0;
...
const tmpDir = path.join(BIN_DIR, "_tmp_extract");
    ...
    const findExe = (dir) => { for (const e of fs.readdirSync(dir, { withFileTypes: true })) { const p = path.join(dir, e.name); if (e.isDirectory()) { const r2 = findExe(p); if (r2) return r2; } else if (e.name.toLowerCase() === "sing-box.exe") { fs.copyFileSync(p, exePath); return true; } } return false; };
    if (!findExe(tmpDir)) throw new Error("sing-box.exe not found after extraction");
    try { fs.rmSync(zp, { force: true }); } catch {}
    try { fs.rmSync(tmpDir, { recursive: true, force: true }); } catch {}
    dc = null;
    is = { state: "done", progress: 100, phase: "", error: "" };
    logger.info("singbox.install.done", { path: BUNDLED_BIN });
  } catch (e) { is = { state: "error", progress: 0, phase: "", error: e.message }; logger.error("singbox.install.error", { error: e.message }); }
  return instStat();
}
...
async function socksFetch(url, opts = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(timeout, () => { try { req.destroy(); } catch {} reject(new Error("Proxy timeout")); });
    req.on("error", reject);
    // Уважаю AbortSignal
    if (opts.signal) {
      if (opts.signal.aborted) { req.destroy(); return reject(new Error("Aborted")); }
      opts.signal.addEventListener("abort", () => { req.destroy(); reject(new Error("Aborted")); }, { once: true });
    }
...
module.exports = {
  parseVlessLink, parseProfile, buildSBConfig, normalizeConfig, detectSB, getStatus,
  startProxy, stopProxy, pingProxy,
  installStatus: instStat, installSingBox: installSB,
  getProxyAgent, getProxyUrl, proxyFetch,
  getSavedVless, saveVless, deleteVless,
};
