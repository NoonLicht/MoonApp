...
router.get("/status", (req, res) => {
  try {
    res.json(statusPayload(proxyCore.getCoreStatus()));
  } catch (e) {
...
router.post("/stop", async (req, res) => {
  try {
    res.json(statusPayload(await proxyCore.stopCore()));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
router.get("/install", (req, res) => {
  try {
    res.json(proxyCore.installStatus());
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
router.post("/install/start", async (req, res) => {
  try {
    res.json(await proxyCore.installEngine());
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
router.post("/subscriptions", async (req, res) => {
  try {
    ...
    const refresh = await proxySubs
      .refreshSubscription(id)
      .catch((e) => ({ error: e.message, added: 0 }));
    res.json({ id, refresh });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
router.post("/subscriptions/:id/refresh", async (req, res) => {
  try {
    res.json(await proxySubs.refreshSubscription(Number(req.params.id)));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
router.delete("/subscriptions/:id", (req, res) => {
  try {
    res.json(stmts.psubDelete.run(Number(req.params.id)));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
router.get("/nodes", (req, res) => {
  try {
    res.json(stmts.pnodeAll.all().map(summarizeNode));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
router.delete("/nodes/hidden", (req, res) => {
  try {
    ...
    const rows = stmts.pnodeAll
      .all()
      .filter((n) => n.is_excluded && (sub == null || n.sub_id === sub));
...
router.post("/pages", (req, res) => {
  try {
    ...
    if (!route || typeof route !== "string")
      return res.status(400).json({ error: "route_required" });
...
function summarizeNode(row) {
  ...
  try {
    ...
    /* пустой/битый JSON — поля ниже останутся null */
  }
  return {
    id: row.id,
    subId: row.sub_id,
