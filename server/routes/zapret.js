...
router.get("/update", async (req, res) => {
  try {
    res.json(await zapret.checkUpdate());
  } catch (e) {
...
router.post("/start", async (req, res) => {
  try {
    res.json(await zapret.start(req.body || {}));
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
});
...
router.post("/service", async (req, res) => {
  try {
    res.json(
      await zapret.serviceAction(String(req.body?.action || "status"), req.body?.strategyId),
    );
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
...
router.post("/diagnostics", async (req, res) => {
  try {
    res.json(await zapret.runDiagnostics());
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
router.post("/auto-tune", async (req, res) => {
  try {
    res.json(await zapret.autoTune(req.body || {}));
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
});
...
router.get("/lists/:name", (req, res) => {
  try {
    res.json({ name: req.params.name, content: zapret.readList(req.params.name) });
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
});
...
router.put("/lists/:name", (req, res) => {
  try {
    res.json({ ok: zapret.writeList(req.params.name, req.body?.content ?? "") });
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
});
...
router.delete("/profiles/:id", (req, res) =>
  res.json({ ok: !!stmts.bpDelete.run(Number(req.params.id)).changes }),
);
...
router.post("/profiles/:id/activate", async (req, res) => {
  try {
    ...
    const strategyId =
      /:\s*(\S+?)\s*\(/.exec(p.name)?.[1] || zapret.listStrategies()[0]?.id || "general";
    res.json(
      await zapret.start({
        strategyId,
        customArgs: p.custom_args || "",
        mode: p.is_service ? "service" : "process",
      }),
    );
  } catch (e) {
    res.status(400).json({ error: e.message });
...
router.post("/domains", (req, res) => {
  try {
    ...
    try {
      zapret.syncCustomDomains();
    } catch {
      /* движок не развёрнут — домен останется в БД */
    }
...
router.post("/check/stop", async (req, res) => {
  try {
    res.json(await zapret.stopConfigCheck());
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
});
...
router.post("/service-diagnostics", async (req, res) => {
  try {
    res.json(await zapret.runServiceDiagnostics());
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
});
...
router.post("/user-lists", async (req, res) => {
  try {
    res.json(await zapret.fixUserLists());
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
});
