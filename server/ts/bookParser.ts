 ...
 *
 * TS-исходник, как server/ts/convertEngine.ts: компилируется в
 * server/bookParser.js командой `npm run compile:server`, поэтому
 * require("./bookParser") из server/tts.js и роутов работает без изменений.
 */
import fs from "fs";
import path from "path";
import AdmZip from "adm-zip";

/** Глава разобранной книги. */
export interface Chapter {
  title: string;
  text: string;
}

/** Итог разбора книги (format/encoding добавляются на входе parseBook/parseTxt). */
...
/**
 * iconv-lite держим необязательным: он не объявлен в зависимостях (приходит
 * транзитивно) и нужен только для .txt в cp1251/cp866 — есть фолбэк на utf8.
 */
function safeIconv(): { decode(buf: Buffer, enc: string): string } | null {
  try {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    return require("iconv-lite") as { decode(buf: Buffer, enc: string): string };
  } catch {
    return null;
  }
}
...
function cp866Char(code: number): string {
...
export function detectDecode(buf: Buffer): { text: string; encoding: string } {
...
function parseEpub(buf: Buffer): ParsedBook {
  ...
  const items: Record<string, { href: string }> = {};
  ...
  // Без spine (битый OPF) порядок берём по самим файлам в архиве.
  const order: Array<{ href?: string } | string> = spineIds.length
  ...
  for (const item of order) {
    const entry = zip.getEntry(typeof item === "string" ? item : String(item.href));
    if (!entry) continue;
...
function parseFb2(buf: Buffer): ParsedBook {
...
async function parsePdf(buf: Buffer): Promise<ParsedBook> {
  ...
  // ВНИМАНИЕ (проверено эмпирически): pdf-parse v2 закрыл подпуть lib/ через
  // "exports", поэтому этот require кидает ERR_PACKAGE_PATH_NOT_EXPORTED и
  // разбор PDF падает всегда. Поведение сохранено 1:1 при переводе на TS;
  // лечится переходом на require("pdf-parse") (см. tests/bookParser.test.ts).
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const pdfParse = require("pdf-parse/lib/pdf-parse.js") as (b: Buffer) => Promise<{
    text?: string;
...
function parseMobi(buf: Buffer): ParsedBook {
...
function parseRtf(buf: Buffer): ParsedBook {
...
export async function parseBook(filePath: string, originalName?: string): Promise<ParsedBook> {
