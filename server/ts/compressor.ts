 ...
 *    NVEncC/QSVEncC/VCEEncC). Всё обнаруживается автоматически (encoders.ts).
 ...
 *
 * TS-исходник, как server/ts/jobStore.ts: компилируется в server/compressor.js
 * командой `npm run compile:server`, поэтому `require("./compressor")` из
 * routes/compressor.js продолжает работать без изменений.
 */
...
import type { HwInfo, HwMethods } from "./encoders";
import logger from "./logger";
import config from "./config";
import { detectFfmpeg } from "./convertEngine";
import type { FfmpegInfo } from "./convertEngine";
...
import { removePath, removeOlderThan } from "./fsUtil";
import { createQueue, trimJobs } from "./jobStore";

const { DIRS } = config;

/** Параметры запуска из тела запроса: multipart отдаёт числа строками. */
interface RawParams {
  codec?: unknown;
  engine?: unknown;
  qualityMode?: unknown;
...
/** Параметры запуска задания: файл из multer + поля multipart. */
interface RawJobInput extends RawParams {
  inputPath: string;
  name?: string;
  size?: number;
}

/** Нормализованные параметры задания (после normalizeParams). */
...
/** Сырой поток из ffprobe: значения приходят строками или числами. */
interface RawStream {
  width?: number;
  height?: number;
  codec_name?: string;
...
/** Колбэк прогресса кодирования: принимает уже прошедшие секунды. */
type ProgressFn = ((sec: number) => void) | null;

/** Задание сжатия: UI опрашивает его состояние. */
export interface CompressorJob extends CompressorParams {
  id: string;
  createdAt: number;
  startedAt: number;
  inputPath: string;
...
/** Пресет сжатия: системные из SYSTEM_PRESETS + пользовательские из settings. */
...
const jobs = new Map<string, CompressorJob>();
const JOB_LIMIT = 30;
...
function probeVideoInfo(ffprobe: string, file: string): Promise<VideoInfo> {
  return new Promise((resolve) => {
    ...
    execFile(
      ...
      (err, stdout) => {
        ...
        try {
          const streams = (JSON.parse(String(stdout)) as { streams?: RawStream[] }).streams || [];
          const s =
            streams
...
function emitProgress(s: string, onProgress: ProgressFn, totalSec: number): void {
...
function setStage(job: CompressorJob, stage: string, weightBase: number, weightSpan: number): void {
  job.stage = stage;
...
function tracker(job: CompressorJob, totalSec: number): (sec: number) => void {
  return (sec: number) => {
...
const SYSTEM_PRESETS: CompressorPreset[] = [
  {
    id: "discord",
    codec: "av1",
    engine: "auto",
...
function defaultSpeed(engine: string): string {
  switch (engine) {
    case "svtav1":
    case "av1an":
      return "6";
...
function normalizeParams(raw: RawParams): CompressorParams {
  const p: CompressorParams = {
    codec: CODECS.includes(String(raw.codec)) ? String(raw.codec) : "av1",
    engine: ENGINES.includes(String(raw.engine)) ? String(raw.engine) : "auto",
    qualityMode: QUALITY_MODES.includes(String(raw.qualityMode)) ? String(raw.qualityMode) : "crf",
    ...
    targetHeight: HEIGHTS.includes(String(raw.targetHeight))
      ...
      : "original",
    audio: ["copy", "aac", "opus"].includes(String(raw.audio)) ? String(raw.audio) : "aac",
...
function engineCandidates(engine: string, codec: string, methods: HwMethods): string[] {
  const perCodec: Record<string, string[]> = {
  ...
  const ok = list.filter((e) => methods[e as keyof HwMethods] === true);
...
function quoteCmd(cmd: string, args: string[]): string {
  const q = (s: string) =>
    /\s/.test(String(s)) ? `"${String(s).replace(/"/g, '\\"')}"` : String(s);
  return [cmd, ...args.map(q)].join(" ");
}

// Флаги качества под энкодер (CRF-подобное поведение).
function qualityFlags(enc: string, crf: number): string[] {
...
function speedArgs(enc: string, speed: string): string[] {
  if (!speed) return [];
...
function ffmpegArgs(
  job: CompressorJob,
  ffEnc: string,
  opts: { pass?: number; outFile?: string } = {},
): string[] {
  ...
  if (opts.pass === 1) {
    ...
    args.push("-shortest", String(opts.outFile));
  }
  return args;
}

// rigaya-обёртка (NVEncC/QSVEncC/VCEEncC): кодек + CQP/VBR + аудио.
function rigayaArgs(job: CompressorJob, opts: { outFile: string }): string[] {
...
function av1anArgs(job: CompressorJob, opts: { outFile: string }): string[] {
...
async function runJob(job: CompressorJob): Promise<void> {
  ...
  try {
    ...
    for (const method of cands) {
      try {
        ...
        logger.warn("compressor.fallback", {
          method,
...
async function encodeWithMethod(
  job: CompressorJob,
  method: string,
  hw: HwInfo,
  ff: FfmpegInfo,
  ctx: { outFile: string; passLog: string; onProg: ProgressFn },
): Promise<string> {
  const { outFile, onProg } = ctx;
  // При ff.found путь к ffmpeg всегда есть, но тип поля допускает null.
  const ffmpeg = String(ff.ffmpeg);
  ...
  if (method === "rav1e" && !hw.ffmpegEncoders.includes("librav1e")) {
    ...
    const exe = String(hw.externals.rav1e);
  ...
  for (const ffEnc of avail) {
    try {
      ...
      lastErr = e;
      logger.warn("compressor.ffEncFallback", {
        ffEnc,
...
function getJob(id: string): CompressorJob | null {
