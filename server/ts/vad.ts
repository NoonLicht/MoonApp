...
/** Опции детектора (полные пояснения — в JSDoc конструктора VadBufferManager). */
...
/** Измерители по кадрам чанка (frameStats): средний/пиковый RMS и статистика ZCR. */
...
/** Класс кадра внутри чанка: речь/не речь и его измерения (нужен для обрезки краёв). */
interface FrameInfo {
  speech: boolean;
  rms: number;
  zcr: number;
...
/** Метрики отрезка: dBFS, доля речи, «тональность» и стационарность шума. */
...
/** Готовый чанк: аудио для Whisper + метрики для БД и UI. */
export interface VadChunk extends ChunkMetrics {
  startMs: number;
  endMs: number;
  speechMs: number;
  samples: Int16Array;
...
/** Отброшенный отрезок: UI показывает его как «пропуск» с причиной и уровнем. */
...
/** Счётчики работы детектора (уходят в UI вместе со статусом сессии). */
...
/** Состояние детектора для панели настроек (metrics()). */
...
/** Итог обрезки краёв чанка по классам кадров (_trimEdges). */
...
export const FRAME_MS = 30; // длительность фрейма VAD

export class VadBufferManager {
  // --- Настройки (в конструкторе приводятся к допустимым рамкам) ---
  sampleRate: number;
  silenceMs: number;
  minChunkMs: number;
  maxChunkMs: number;
  ...
  zcrLimit: number;
  quietWindow: number;
  // --- Состояние автоподстройки порога ---
  steadyRun: number;
  recentRms: number[];
  floorSeenAt: number;
  quietRms: number[];
  ...
  frameBuf: Int16Array;
  frameFill: number;
  // --- Буфер текущего чанка ---
  pending: Int16Array[];
  chunkInfo: FrameInfo[];
  speechFrames: Int16Array[];
  chunkStartMs: number;
  chunkSpeechMs: number;
  lastSpeechMs: number;
  silenceRunMs: number;
   ...
   */
  _updateNoiseFloor(rms: number): void {
   ...
   */
  pass(pcm: Int16Array, startMs = 0): VadChunk[] {
  ...
  _pushPreRoll(frame: Int16Array, rms: number): void {
  ...
  _processFrame(frame: Int16Array): VadChunk | null {
  ...
  _metricsOf(frames: Int16Array[], speechMs: number, durMs: number): ChunkMetrics {
  ...
  _rejectReason(m: ChunkMetrics, durMs: number): string {
   ...
   * первого слова.
   */
  _trimEdges(frames: Int16Array[], info: FrameInfo[], startMs: number): TrimResult | null {
  ...
  _splitAt(cut: number): VadChunk | null {
...
function framesToMs(n: number): number {
 ...
 */
export function zcrOf(frame: Int16Array): number {
  let crosses = 0;
 ...
 */
export function frameStats(frames: Int16Array[]): FrameStats {
 ...
 */
function padTail(samples: Int16Array, sampleRate: number, padMs: number): Int16Array {
