...
export default function LectureAudioPanel({
  ...
  const vad = draft?.vad;
  const thresholdDb =
    Math.round(20 * Math.log10(Math.max(0.0005, vad?.rmsThreshold ?? 0.008)) * 10) / 10;
  ...
  const body = (
    <div className={inline ? "lecs-inline-body" : "lecs-panel"}>
      <div className="lecs-head">
        ...
        <span
          className={`lecs-pill ${levelDb >= (vad?.rmsThreshold != null ? 20 * Math.log10(vad.rmsThreshold) : -42) ? "on" : "off"}`}
        >
      ...
      <div className="lecs-block">
        ...
        {devices.length === 0 && (
          <div className="lecs-warn">
            <AlertTriangle size={13} />
            <span>{t("lecture.audio.micNone")}</span>
          </div>
        )}
      </div>
      ...
      <div className="lecs-block">
        ...
        <div className="leca-row">
          <span className="lecs-dim leca-label">
            {t("lecture.audio.gain", { v: Number(vad ? (draft?.micGain ?? 1) : 1).toFixed(1) })}
          </span>
          <input
            type="range"
            min="0.5"
            ...
            disabled={recording}
            onChange={(e) =>
              setDraft((d) => (d ? { ...d, micGain: parseFloat(e.target.value) } : d))
            }
            style={{ width: 170 }}
          />
      ...
      <div className="lecs-block">
        ...
        <div className="leca-row">
          <span className="lecs-dim leca-label">
            {t("lecture.audio.manualThreshold", { db: thresholdDb })}
          </span>
          <input
            type="range"
            min="-66"
            ...
            onChange={(e) => {
              ...
              setDraft((d) =>
                d ? { ...d, vad: { ...d.vad, rmsThreshold: Math.pow(10, db / 20) } } : d,
              );
            }}
            style={{ width: 170 }}
          />
        ...
        <div className="leca-row">
          <span className="lecs-dim leca-label">
            {t("lecture.audio.factor", { v: Number(vad?.thresholdFactor ?? 3).toFixed(1) })}
          </span>
          <input
            type="range"
            min="1.5"
            ...
            onChange={(e) =>
              setDraft((d) =>
                d ? { ...d, vad: { ...d.vad, thresholdFactor: parseFloat(e.target.value) } } : d,
              )
            }
            style={{ width: 170 }}
          />
        ...
        <div className="leca-row">
          <span className="lecs-dim leca-label">
            {t("lecture.audio.minRatio", {
              v: Math.round(Number(vad?.minSpeechRatio ?? 0.15) * 100),
            })}
          </span>
          <input
            type="range"
            ...
            onChange={(e) =>
              setDraft((d) =>
                d ? { ...d, vad: { ...d.vad, minSpeechRatio: parseFloat(e.target.value) } } : d,
              )
            }
            style={{ width: 170 }}
          />
          ...
          <div className="leca-diag">
            <span>
              <Badge tone="neutral" mono>
                {t("lecture.audio.diagThreshold")}
              </Badge>{" "}
              {metrics.thresholdDb} dBFS
            </span>
            <span>
              <Badge tone="neutral" mono>
                {t("lecture.audio.diagNoise")}
              </Badge>{" "}
              {metrics.noiseFloorDb} dBFS
            </span>
            <span>
              <Badge tone="neutral" mono>
                {t("lecture.audio.diagSpeech")}
              </Badge>{" "}
              {metrics.stats.speechFrames}/{metrics.stats.frames}
            </span>
            <span>
              <Badge tone="neutral" mono>
                {t("lecture.audio.diagNoiseFrames")}
              </Badge>{" "}
              {metrics.stats.noiseFrames}
            </span>
            <span>
              <Badge tone="neutral" mono>
                {t("lecture.audio.diagSkipped")}
              </Badge>{" "}
              {Math.round(metrics.stats.skippedMs / 1000)} {t("lecture.audio.sec")}
            </span>
            <span>
              <Badge tone="neutral" mono>
                {t("lecture.audio.diagChunks")}
              </Badge>{" "}
              {metrics.stats.chunks}
            </span>
          </div>
...
export function LevelMeter({
  ...
  return (
    <span
      className={`lec-meter ${quiet ? "quiet" : "ok"}`}
      title={t("lecture.audio.meterHint", { db, thr: thresholdDb })}
    >
      <span className="lec-meter-bar">
        <i style={{ width: `${pct}%` }} />
      </span>
