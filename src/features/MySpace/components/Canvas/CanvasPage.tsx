...
let seq = 0;
const uid = (p: string) => `${p}_${Date.now().toString(36)}${(seq++).toString(36)}${Math.floor(Math.random() * 1296).toString(36)}`;
...
function CanvasInner() {
  ...
  const { screenToFlowPosition, zoomIn, zoomOut, setViewport, getViewport, fitView } = useReactFlow();

  const [boardName, setBoardName] = useState("Untitled Holst 01");
  const [tool, setTool] = useState<CanvasTool>("select");
  const [shape, setShape] = useState<ShapeKind>("rounded");
  const [sticker, setSticker] = useState("👍");
  const [lineProps, setLineProps] = useState<LineProps>({ style: "bezier", dash: "solid", arrowStart: false, arrowEnd: true, animated: false });
  ...
  const frameDrag = useRef<{ frameId: string; children: string[]; starts: Record<string, { x: number; y: number }>; origin: { x: number; y: number } } | null>(null);
  ...
  const pushHistory = useCallback((snapshot?: { nodes: Node[]; edges: Edge[] }) => {
    past.current = [...past.current.slice(-49), snapshot || { nodes: clean(nodes), edges: clean(edges) }];
    future.current = [];
    forceHist((v) => v + 1);
  }, [nodes, edges]);
  ...
  /* ── node factory ── */
  const addNode = useCallback((type: string, pos: { x: number; y: number }, data: any, extra?: Partial<Node>): Node => {
  ...
  const setData = useCallback((id: string, patch: any) => {
    setNodes((ns) => ns.map((n) => (n.id === id ? { ...n, data: { ...n.data, ...patch } } : n)));
  }, [setNodes]);

  const addConnectorEdge = useCallback((source: string, sourceHandle: string | null, target: string, targetHandle: string | null) => {
    setEdges((es) => [...es, {
      id: uid("e"),
      source,
      target,
  ...
  const save = useCallback(async () => {
    ...
    } catch (e) { console.error("holst save failed", e); }
  ...
  useEffect(() => {
    const onBeforeUnload = () => { if (dirtyRef.current) save(); };
  ...
  const spawnForTool = useCallback((flowPos: { x: number; y: number }) => {
    ...
    switch (tool) {
      ...
      case "text":
        ...
        break;
      case "shape":
        addNode("shape", flowPos, { shape, label: "Label", fill: STICKY_COLORS[Math.floor(Math.random() * STICKY_COLORS.length)], stroke: "#16181f", w: 160, h: 90, setData });
        break;
      case "frame":
        addNode("frame", flowPos, { label: "Frame", color: "#3fc7ab", w: 420, h: 320, setData });
        break;
      case "task":
        addNode("task", flowPos, { title: "New task", status: "todo", subtasks: [{ text: "step", done: false }], setData });
        break;
      case "sticker": {
        if (sticker.startsWith("pill:")) {
          const pill = sticker.slice(5);
          addNode("shape", flowPos, { shape: "rounded", label: pill, fill: pill === "CRITICAL" ? "#fed7aa" : "#bbf7d0", stroke: "#16181f", w: 130, h: 44, setData }, { style: { width: 130, height: 44 } });
        } else {
  ...
  const onConnectEnd = useCallback((event: MouseEvent | TouchEvent, state: any) => {
    ...
    setConnectMenu({ x, y, fx: flow.x, fy: flow.y, source: fromNode.id, handle: state.fromHandle?.id || "r" });
  }, [screenToFlowPosition]);

  const connectSpawn = useCallback((kind: "note" | "task" | "decision") => {
  ...
  const onDrop = useCallback((event: React.DragEvent) => {
    ...
    pushHistory();
    addNode("md", flow, { path: mdPath, name: mdPath.split("/").pop() || mdPath, loaded: false, setData });
  ...
  const onPenDown = useCallback((e: React.PointerEvent) => {
    ...
    setPenPreview({ x1: penOrigin.current.x, y1: penOrigin.current.y, x2: penOrigin.current.x, y2: penOrigin.current.y });
  }, [tool]);

  const onPenMove = useCallback((e: React.PointerEvent) => {
    if (tool !== "pen" || penPoints.current.length === 0) return;
    const rect = wrapRef.current!.getBoundingClientRect();
    penPoints.current.push({ x: e.clientX - rect.left, y: e.clientY - rect.top });
    setPenPreview({ x1: penOrigin.current!.x, y1: penOrigin.current!.y, x2: e.clientX - rect.left, y2: e.clientY - rect.top });
  }, [tool]);

  const onPenUp = useCallback(() => {
    if (tool !== "pen" || penPoints.current.length < 2) { setPenPreview(null); penPoints.current = []; return; }
    ...
    pushHistory();
    addNode("stroke", tl, { points: pts, color: "#f0a63d", width: 3, opacity: 0.9, w, h }, { style: { width: w, height: h }, draggable: false });
    penPoints.current = [];
    setPenPreview(null);
  ...
  const exportJson = useCallback(() => {
    const doc = {
      version: 2, name: boardName,
      nodes: clean(nodes), edges: clean(edges),
      viewport: getViewport(), updatedAt: new Date().toISOString(),
    };
  ...
  const exportImage = useCallback(async (fmt: "png" | "svg") => {
    const viewportEl = wrapRef.current?.querySelector(".react-flow__viewport") as HTMLElement | null;
    ...
    const cssProps = ["position", "left", "top", "width", "height", "margin", "padding", "background-color", "background",
      "border", "border-radius", "box-shadow", "color", "font-family", "font-size", "font-weight", "line-height",
      "text-align", "display", "flex-direction", "align-items", "justify-content", "gap", "overflow", "opacity",
      "transform", "stroke", "fill", "stroke-width", "stroke-dasharray", "letter-spacing", "white-space"];
    ...
    const offX = bbox.left - wrapRect.left, offY = bbox.top - wrapRect.top;
  ...
  const onExport = useCallback((fmt: "png" | "svg" | "json") => {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "!") { e.preventDefault(); fitView({ padding: 0.3, duration: 250 }); return; }
      if (e.key === "/") {
        e.preventDefault();
        const rect = wrapRef.current!.getBoundingClientRect();
      ...
      const map: Record<string, CanvasTool> = {
        ...
        a: "connector", f: "frame", k: "task", p: "pen", e: "sticker",
      };
      const k = e.key.toLowerCase();
      if (map[k]) { setTool(map[k]); setSlash(null); }
    };
  ...
  if (!loaded) {
    return <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: "var(--text-tertiary)", fontSize: 13 }}>Loading canvas…</div>;
  }

  const cursor =
    tool === "hand" ? "grab" :
    tool === "pen" ? "crosshair" :
    tool === "select" ? "default" : "copy";
  ...
  return (
    ...
    >
      ...
      {/* pen live preview */}
      {penPreview && (
        <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none", zIndex: 45 }}>
          <line x1={penPreview.x1} y1={penPreview.y1} x2={penPreview.x2} y2={penPreview.y2} stroke="#f0a63d" strokeWidth={3} strokeLinecap="round" opacity={0.9} />
        </svg>
      )}
      ...
      {slash && (
        <div className="holst-slash" style={{ left: slash.x, top: slash.y }} onMouseDown={(e) => e.stopPropagation()}>
          {([
            ["note", "📝", "Note"],
            ["sticky", "🟡", "Sticky note"],
            ["task", "✅", "Task"],
      ...
      {connectMenu && (
        <div className="holst-pop" style={{ left: connectMenu.x, top: connectMenu.y }} onMouseDown={(e) => e.stopPropagation()}>
          <button className="holst-pop-item" onClick={() => connectSpawn("note")}>📝 Create connected Note</button>
          <button className="holst-pop-item" onClick={() => connectSpawn("task")}>✅ Create connected Task</button>
          <button className="holst-pop-item" onClick={() => connectSpawn("decision")}>💠 Create Decision Node</button>
        </div>
      )}
      ...
      {tool !== "select" && (
        <div className="holst-canvas-hint">
          ...
          {tool === "sticker" && `Click to place ${sticker.startsWith("pill:") ? sticker.slice(5) + " pill" : sticker}`}
        </div>
      )}

      {showTemplates && <TemplatesModal onClose={() => setShowTemplates(false)} onSelect={applyTemplate} />}
    </div>
  );
}
