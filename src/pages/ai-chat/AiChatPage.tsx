...
import MsgList, { type MsgStats } from "@/pages/ai-chat/parts/MsgList";
...
import {
  ...
  exportChatMd,
  exportChatJson,
} from "@/pages/ai-chat/lib/chatUtils";
...
const HYPER_PRESETS: { id: string; label: string; cfg: Partial<ChatCfg> }[] = [
  {
    id: "precise",
    label: "🎯",
    cfg: { temperature: 0.2, topP: 0.9, frequencyPenalty: 0, presencePenalty: 0 },
  },
  {
    id: "balance",
    label: "⚖️",
    cfg: { temperature: 0.7, topP: 1, frequencyPenalty: 0, presencePenalty: 0 },
  },
  {
    id: "creative",
    label: "🎨",
    cfg: { temperature: 1.2, topP: 1, frequencyPenalty: 0.4, presencePenalty: 0.4 },
  },
];
...
export default function AiChatPage() {
  ...
  const setChatCfg = useCallback((patch: Partial<ChatCfg>) => {
    ...
    try {
      const touched = new Set<string>(
        JSON.parse(localStorage.getItem("aichat.cfg.touched") || "[]"),
      );
  ...
  const [presetDraft, setPresetDraft] = useState<{ open: boolean; name: string }>({
    open: false,
    name: "",
  });
  ...
  const [arena, setArena] = useState<{
    ...
    a: "",
    b: "",
    aText: { text: "", done: false },
    bText: { text: "", done: false },
  });
  ...
  useEffect(() => {
    ...
    if (last.provider || last.model) {
      setChatCfgState((s) => ({
        ...s,
        provider: last.provider || s.provider,
        model: last.model || s.model,
      }));
    }
      ...
      api
        ...
        .then((s: any) => {
          ...
          setChatCfgState((p: ChatCfg) => ({
            ...
            temperature:
              touched.has("temperature") || typeof c.temperature !== "number"
                ? p.temperature
                : c.temperature,
            maxTokens:
              touched.has("maxTokens") || typeof c.maxTokens !== "number"
                ? p.maxTokens
                : c.maxTokens,
            streaming:
              touched.has("streaming") || typeof c.stream !== "boolean" ? p.streaming : c.stream,
          }));
        })
        .catch(() => {
          /* настройки недоступны — остаётся localStorage */
        });
    } catch {
  ...
  useEffect(() => {
    saveLast(chatCfg.provider, chatCfg.model);
  }, [chatCfg.provider, chatCfg.model]);
  ...
  const provider = useMemo(
    () => providers.find((p) => p.id === chatCfg.provider),
    [providers, chatCfg.provider],
  );
  ...
  const groupedConvs = useMemo(() => {
    ...
    const g2 = rest.filter(
      (c) =>
        new Date(c.updated_at.replace(" ", "T")) >= yesterday &&
        new Date(c.updated_at.replace(" ", "T")) < dayStart,
    );
        ...
        (ev) => {
          if (ev.type === "token" && chatCfg.streaming) {
            ...
            if (ev.stats) {
              setLastStats({
                ms: ev.stats.ms,
                tokens: Math.round(ev.stats.tokensApprox / (ev.stats.ms / 1000)),
              });
            }
            setStreamingText("");
            setSending(false);
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      >
        ...
        <Field label={t("aichat.model")}>
          <Select
            value={chatCfg.model}
            onChange={(e) => setChatCfg({ model: e.target.value })}
            options={models}
          />
        </Field>
      </ToolbarMenu>
  ...
  const regenerate = async () => {
    ...
    try {
      await api.chatTruncateFrom(activeId, lastUser.id);
    } catch {
      return;
    }
    setMessages((m) => m.filter((x) => (x.id || 0) < lastUser.id!));
  ...
  const saveEdit = async () => {
    ...
    try {
      await api.chatTruncateFrom(activeId, target.id!);
    } catch {
      return;
    }
  ...
  const addImageFile = (file: File) => {
    ...
    reader.onload = () =>
      setAttachments((a) => [
        ...a,
        { name: file.name || "image.png", type: "image", data: reader.result as string },
      ]);
      ...
      {settingsOpen && (
        <Glass className="settings-drawer">
          <div className="hyperparams-panel">
            <div className="hyper-presets">
              {HYPER_PRESETS.map((hp) => (
                <button key={hp.id} title={hp.id} onClick={() => setChatCfg(hp.cfg)}>
                  {hp.label}
                </button>
              ))}
            </div>
            <label>
              ...
              <input
                ...
                max="2"
                step="0.05"
                value={chatCfg.temperature}
                onChange={(e) => setChatCfg({ temperature: parseFloat(e.target.value) })}
              />
              <span className="val">{chatCfg.temperature.toFixed(2)}</span>
            </label>
            <label>
              <span>Top P</span>
              <input
              ...
              />
              <span className="val">{chatCfg.topP.toFixed(2)}</span>
            </label>
            <label>
              <span>Freq P</span>
              <input
                ...
                max="2"
                step="0.1"
                value={chatCfg.frequencyPenalty}
                onChange={(e) => setChatCfg({ frequencyPenalty: parseFloat(e.target.value) })}
              />
              <span className="val">{chatCfg.frequencyPenalty.toFixed(1)}</span>
            </label>
            <label>
              <span>Pres P</span>
              <input
                ...
                max="2"
                step="0.1"
                value={chatCfg.presencePenalty}
                onChange={(e) => setChatCfg({ presencePenalty: parseFloat(e.target.value) })}
              />
              <span className="val">{chatCfg.presencePenalty.toFixed(1)}</span>
            </label>
            <label>
          ...
          <div className="sys-panel">
            <select
              value={allPresets.some((p) => p.name === sysPreset) ? sysPreset : ""}
              onChange={(e) => handleSysPreset(e.target.value)}
            >
              {customPresets.length > 0 && (
                <optgroup label={t("aichat.customPresets")}>
                  {customPresets.map((p) => (
              ...
              {presetDraft.open ? (
                <>
                  <input
                    ...
                    placeholder={t("aichat.presetName")}
                    style={{ flex: 1, minWidth: 0 }}
                  />
                  <Btn variant="primary" icon={Check} onClick={addCustomPreset}>
                    {t("aichat.presetSave")}
                  </Btn>
                  <Btn icon={X} onClick={() => setPresetDraft({ open: false, name: "" })}>
                    {t("aichat.cancel")}
                  </Btn>
                </>
              ) : (
                <>
                  <Btn icon={Plus} onClick={() => setPresetDraft({ open: true, name: "" })}>
                    {t("aichat.presetNew")}
                  </Btn>
                  {customPresets.map((p) => (
                    <button
                      key={p.name}
                      className="sys-preset-del"
                      onClick={() => delCustomPreset(p.name)}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <Btn icon={Download} onClick={() => doExport("md")}>
              {t("aichat.exportMd")}
            </Btn>
            <Btn icon={Code2} onClick={() => doExport("json")}>
              {t("aichat.exportJson")}
            </Btn>
          </div>
        </Glass>
            ...
            {paletteModels.length > 0 && (
              <>
                ...
                {paletteModels.map((m) => (
                  <button
                    key={m}
                    className={`palette-item ${m === chatCfg.model ? "is-active" : ""}`}
                    onClick={() => {
                      setChatCfg({ model: m });
                      setPalette(null);
                    }}
      ...
      <div className={`chat-side ${sideOpen ? "" : "is-collapsed"} ${isNarrow ? "is-narrow" : ""}`}>
        ...
        <div className="chat-sidebar">
          <Glass className="chat-conv-list" style={{ padding: 8 }}>
                ...
                {g.items.map((c) =>
                  renaming?.id === c.id ? (
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter")
                          renameConv(c.id, (e.target as HTMLInputElement).value);
                    ...
                    <div
                      key={c.id}
                      className={`chat-conv ${c.id === activeId ? "is-active" : ""}`}
                      onClick={() => {
                        setActiveId(c.id);
                        if (isNarrow) setSideOpen(false);
                      }}
                      onContextMenu={(e) =>
                        menu.open(e, [
                          ...
                          },
                          {
                            label: t("ctx.rename"),
                            icon: Pencil,
                            onClick: () => setRenaming({ id: c.id, text: c.title }),
                          },
                          {
                            label: c.pinned ? t("ctx.unpin") : t("ctx.pin"),
                    ...
                    >
                      {c.pinned ? (
                        <Pin size={11} style={{ color: "var(--amber)", flexShrink: 0 }} />
                      ) : null}
                      <span
                        style={{
                          overflow: "hidden",
                      ...
                      <span className="x">
                        <IconBtn
                          ...
                          title={c.pinned ? t("aichat.unpin") : t("aichat.pin")}
                        />
                        <IconBtn
                          icon={Pencil}
                          size={12}
                          onClick={(e) => {
                            e.stopPropagation();
                            setRenaming({ id: c.id, text: c.title });
                          }}
            ...
            {convs.length === 0 && (
              <div className="muted-sm" style={{ padding: 8 }}>
                {t("aichat.noChats")}
              </div>
            )}
          </Glass>
                  ...
                  return (
                    <div key={side} className="arena-col">
                      <div className="arena-col-head">
                        ...
                        <select
                          value={mdl}
                          onChange={(e) => setArena((a) => ({ ...a, [side]: e.target.value }))}
                        >
                          {models.map((m) => (
                                ...
                                seg.type === "code" ? (
                                  ...
                                  <span
                                    ...
                                    dangerouslySetInnerHTML={{
                                      __html: sanitizeHtml(renderInlineMd(seg.text)),
                                    }}
                                  />
                                ),
                              )
                        ...
                        {st.done && st.text && (
                          <>
                            <div className="arena-stats">
                              {t("aichat.chars", { n: st.text.length })} · {mdl}
                            </div>
                            {!st.text.startsWith("⚠") && (
                              <Btn
                                variant="primary"
                                icon={Check}
                                onClick={() => chooseArenaSide(side)}
                              >
          ...
          >
            {attachments.length > 0 && (
              <div
                ...
                style={{ position: "absolute", bottom: "100%", left: 16, marginBottom: 4 }}
              >
                {attachments.map((a, i) => (
                  <div key={i} className="chat-attachment">
                    {a.type === "image" ? (
                      <img src={a.data} alt="" style={{ width: 20, height: 20, borderRadius: 4 }} />
                    ) : (
                      <FileText size={14} />
                    )}
                    <span>{a.name}</span>
            ...
            <textarea
              ...
              placeholder={
                activeConv
                  ? t("aichat.placeholder", { provider: activeConv.provider })
                  : t("aichat.createFirst")
              }
              value={input}
            ...
            <IconBtn
              ...
              title={t("aichat.attach")}
            />
            <IconBtn
              icon={Mic}
              onClick={toggleMic}
              title={listening ? t("aichat.listening") : t("aichat.voiceInput")}
