...
export default function ProxyPanel({ onClose }: ProxyPanelProps) {
  ...
  const loadAll = async () => {
    try {
      setCore(await api.proxyCoreStatus());
    } catch {
      /* бэкенд ещё поднимается */
    }
    try {
      setSubs(await api.proxyCoreSubscriptions());
    } catch {
      /* */
    }
    try {
      setNodes(await api.proxyCoreNodes());
    } catch {
      /* */
    }
    try {
      setRules(await api.proxyCorePages());
    } catch {
      /* */
    }
  };
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      try {
        ...
        /* временная ошибка — попробуем на следующем тике */
      }
    }, 700);
  ...
  const applySession = (url: string | null) => {
    try {
      (window as any).appBridge?.applyProxySession?.(url ? { proxyRules: url } : null);
    } catch {
      /* dev/браузер */
    }
  };
  ...
  const errorText = (raw: string): string => {
    ...
    if (raw.startsWith("port_busy:"))
      return t("proxy.errPortBusy", { port: raw.slice("port_busy:".length) });
    return raw;
  };
  ...
  const testLatency = async () => {
    ...
    try {
      ...
      setLat(r);
      if (r.state === "offline" || r.state === "blocked")
        setError(r.error || t("proxy.pingBlocked"));
    } catch (e) {
      setError((e as Error).message);
    }
    setBusy("");
  ...
  const selectNode = async (id: number) => {
    setBusy("select");
    try {
      await api.proxyCoreSelectNode(id);
      await loadAll();
    } catch (e) {
      setError((e as Error).message);
    }
  ...
  const refreshSub = async (id: number) => {
    ...
    setError("");
    try {
      await api.proxyCoreRefreshSubscription(id);
      await loadAll();
    } catch (e) {
      setError((e as Error).message);
    }
  ...
  const deleteSub = async (id: number) => {
    ...
    setError("");
    try {
      await api.proxyCoreDeleteSubscription(id);
      await loadAll();
    } catch (e) {
      setError((e as Error).message);
    }
  ...
  const hideNode = async (id: number) => {
    ...
    setError("");
    try {
      await api.proxyCoreHideNode(id);
      await loadAll();
    } catch (e) {
      setError((e as Error).message);
    }
  ...
  const restoreNode = async (id: number) => {
    ...
    setError("");
    try {
      await api.proxyCoreRestoreNode(id);
      await loadAll();
    } catch (e) {
      setError((e as Error).message);
    }
  ...
  const pingAll = async (subId: number) => {
    setError("");
    try {
      setPing(await api.proxyCorePingNodes({ subId }));
    } catch (e) {
      setError((e as Error).message);
    }
  };
  ...
  const stopPing = async () => {
    try {
      setPing(await api.proxyCorePingCancel());
    } catch (e) {
      setError((e as Error).message);
    }
  };
  ...
  const isProxied = (route: string) => {
    const r = ruleOf(route);
    return r ? r.is_proxied === 1 : true;
  };
  ...
  const togglePage = async (route: string) => {
    ...
    setError("");
    try {
      await api.proxyCoreSetPage(route, !isProxied(route));
      setRules(await api.proxyCorePages());
    } catch (e) {
      setError((e as Error).message);
    }
  ...
  const renderNode = (n: ProxyNode) => (
    <div
      key={n.id}
      className={`proxy-node ${n.isSelected ? "active" : ""}${n.isExcluded ? " excluded" : ""}`}
    >
      <button
      ...
      >
        ...
        <span className={`proxy-chip-type tone-${PROTO_TONE[n.protocol] || "neutral"}`}>
          {(n.protocol || "").slice(0, 5).toUpperCase()}
        </span>
      ...
      {n.isExcluded ? (
        <button
          ...
          title={t("proxy.restoreNode")}
          disabled={busy === "hide:" + n.id}
          onClick={() => void restoreNode(n.id)}
        >
          <Undo2 size={13} />
        </button>
      ) : (
        <button
          className="proxy-paste"
          title={t("proxy.hideNode")}
          disabled={busy === "hide:" + n.id}
          onClick={() => void hideNode(n.id)}
        >
          <Trash2 size={13} />
        </button>
      )}
  ...
  const pingClass = (n: ProxyNode) =>
    n.pingMs == null ? "blocked" : n.pingMs <= 300 ? "online" : "degraded";
      ...
      <div className="proxy-panel">
        ...
        <div className="proxy-panel-header">
          <div className="proxy-panel-title">
            ...
            <div className="proxy-title-text">
              ...
              <div className="proxy-h1">
                {core?.node
                  ? core.node.tag || core.node.server
                  : `${t("proxy.port")} ${core?.socksPort || 10808}`}
              </div>
            </div>
          </div>
          <span className={`proxy-status ${isOk ? "on" : "off"}`}>
            ...
            {isOk
              ? core?.node?.country || lat?.country || t("proxy.connected")
              : t("proxy.disconnected")}
          </span>
          ...
          <div className="proxy-install-section">
            ...
            {core?.install?.state === "error" && (
              <div className="proxy-hint">
                {core.install.error === "download_failed"
                  ? t("proxy.installFailed")
                  : core.install.error}
        ...
        <Field label={t("proxy.profile")}>
          <div className="proxy-link-row">
            <input
              className="proxy-link"
              value={link}
              onChange={(e) => setLink(e.target.value)}
              placeholder={t("proxy.placeholder")}
              spellCheck={false}
            />
            <button className="proxy-paste" onClick={pasteFromClipboard} title={t("proxy.paste")}>
              <Clipboard size={14} />
            </button>
          </div>
        </Field>
        ...
        <div className="proxy-actions">
          <Btn
            ...
            icon={Activity}
            onClick={testLatency}
            disabled={!isOk || busy === "latency"}
          >
            {busy === "latency" ? t("common.loading") : t("proxy.ping")}
            {lat?.latencyMs != null ? ` · ${lat.latencyMs}ms` : ""}
          </Btn>
          <Btn
            variant={isOk ? "danger" : "primary"}
            icon={isOk ? PowerOff : Power}
            onClick={toggleCore}
          ...
          <div className="proxy-lat">
            <span className={`proxy-ping ${latClass}`}>
              {latClass === "online"
                ? t("proxy.pingOnline")
                : latClass === "degraded"
                  ? t("proxy.pingDegraded")
                  : t("proxy.pingBlocked")}
            </span>
            <span className="proxy-lat-targets">
              {lat.targets.map((x) => String(x.status || "—")).join(" / ")}
            </span>
            {lat.ip && (
              <span className="proxy-lat-ip">
                {t("proxy.exitIp")}: {lat.ip}
                {lat.country ? ` · ${lat.country}` : ""}
              </span>
            )}
          </div>
        )}
        ...
        <div className="proxy-saved-block">
          <div className="proxy-block-label">
            {t("proxy.subscriptions")} · {subs.length}
          </div>
          <div className="proxy-save-bar">
            <input
              ...
              placeholder={t("proxy.subName")}
            />
            <input
              className="proxy-name-input"
              value={subUrl}
              onChange={(e) => setSubUrl(e.target.value)}
              placeholder={t("proxy.subscriptionUrl")}
              spellCheck={false}
            />
            <Btn
              variant="secondary"
              icon={Plus}
              onClick={addSub}
              disabled={!subUrl.trim() || busy === "addsub"}
            >
          ...
          {!!ping && (ping.running || ping.done > 0) && (
            <div className="proxy-ping-progress">
              <div className="proxy-ping-bar">
                <span
                  className="proxy-ping-fill"
                  style={{
                    width: `${ping.total ? Math.round((100 * ping.done) / ping.total) : 0}%`,
                  }}
                />
              </div>
              ...
              {ping.running && (
                <button className="proxy-hidden-toggle" onClick={() => void stopPing()}>
                  {t("proxy.pingCancel")}
                </button>
              )}
            </div>
              ...
              <div key={s.id} className="proxy-sub">
                <div className="proxy-sub-head">
                  ...
                  <span className="proxy-sub-meta">
                    {visible.length}
                    {hidden.length > 0 ? ` +${hidden.length}` : ""} · {s.last_updated || ""}
                  </span>
                  <button
                    className="proxy-paste"
                    title={t("proxy.refresh")}
                    disabled={busy === "refresh:" + s.id}
                    onClick={() => void refreshSub(s.id)}
                  >
                  ...
                  <button
                    className="proxy-paste"
                    title={t("proxy.pingAll")}
                    disabled={!!ping?.running || visible.length === 0}
                    onClick={() => void pingAll(s.id)}
                  >
                    <Activity
                      size={13}
                      className={ping?.running && ping.currentId != null ? "spin" : ""}
                    />
                  </button>
                  <button
                    className="proxy-paste"
                ...
                {visible.length === 0 && hidden.length === 0 && (
                  <div className="proxy-hint">{t("proxy.noNodes")}</div>
                )}
                <div className="proxy-node-list">{visible.map(renderNode)}</div>
                {hidden.length > 0 && showHidden && (
                  <>
                    <div className="proxy-hidden-label">
                      {t("proxy.hiddenNodes")} · {hidden.length}
                    </div>
            ...
            {PAGE_IDS.map((route) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="proxy-page-flag">
                    {on ? t("proxy.proxied") : t("proxy.direct")}
                  </span>
                </button>
              );
            })}
