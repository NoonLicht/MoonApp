...
const fmtBitrate = (bps?: number | null) =>
  !bps ? "—" : bps >= 1e6 ? `${(bps / 1e6).toFixed(1)} Mbps` : `${Math.round(bps / 1e3)} kbps`;
...
export default function CompressorPage() {
  ...
  const [presets, setPresets] = useState<{
    system: CompressorPreset[];
    custom: CompressorPreset[];
  }>({ system: [], custom: [] });
    ...
    api
      ...
      .then((h) => {
        ...
        if (!hwApplied.current) {
          ...
          setP((prev) => ({
            ...prev,
            engine: h.recommended.engine,
            codec: h.recommended.codec,
            qualityMode: h.recommended.qualityMode,
            crf: h.recommended.crf,
  ...
  const speedScale = (() => {
    ...
    if (p.engine === "x265" || p.engine === "x264")
      return [
        "ultrafast",
        "superfast",
        "veryfast",
  ...
  const crfOptimal =
    p.engine === "nvenc" || p.engine === "qsv" || p.engine === "amf"
      ? hw?.optimal?.cqp?.[p.codec]
      : hw?.optimal?.crf?.[p.codec];
  const bitrateOptimal =
    hw?.optimal?.bitrate?.[p.targetHeight === "original" ? "1080" : p.targetHeight];
  ...
  const presetActive = (pr: CompressorPreset) => {
    ...
    return (
      q.codec === p.codec &&
      q.engine === p.engine &&
      q.qualityMode === p.qualityMode &&
      crfOk &&
      audioOk &&
      q.targetHeight === p.targetHeight &&
      q.tenBit === p.tenBit
    );
  };
    ...
    <div className="page cmp-page">
      ...
      <div className="cmp-grid">
        ...
        <div className="cmp-left">
          ...
          {file && !done && (
            <Glass
              className="cmp-preview cmp-card"
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {origCodec && (
                  <Badge tone="violet" mono>
                    {origCodec.toUpperCase()}
                  </Badge>
                )}
                {fileMeta && fileMeta.w > 0 && (
                  <Badge tone="neutral">
          ...
          {done && job && (
              ...
              onContextMenu={(e) =>
                menu.open(e, [
                  ...
                  },
                  {
                    ...
                    onClick: async () => {
                      try {
                        const r = await api.compressorReveal(job.id);
                  ...
                  {
                    label: t("cmp.copyCommand"),
                    icon: Copy,
                    onClick: async () => {
                      try {
                        const r = await api.compressorCommand(job.id);
                        copyToClipboard(r.command);
                      } catch {
                        /* */
                      }
                    },
                  ...
                  { separator: true },
                  {
                    label: t("cmp.download"),
                    icon: Download,
                    onClick: () => {
                      window.location.href = api.compressorUrl(job.id, "download");
                    },
                  },
                  {
                    label: t("cmp.delete"),
                    ...
                    danger: true,
                    onClick: async () => {
                      await api.compressorDelete(job.id);
                      pick(null);
                    },
                  },
                ])
            ...
            >
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                <Btn icon={FileVideo} onClick={() => pick(null)}>
                  {t("cmp.newVideo")}
                </Btn>
                <Btn icon={SlidersHorizontal} onClick={closeJob}>
                  {t("cmp.recompress")}
                </Btn>
              </div>
            </Glass>
          ...
          {defect && (
            <Glass>
              <span style={{ color: "var(--coral)" }}>{defect}</span>
            </Glass>
          )}
        </div>
        ...
        <div className="cmp-right">
          {rec && hw?.ffmpeg.found && (
            ...
            >
              ...
              <span className="muted-sm">
                {t("cmp.recommendFor", {
                  hw: rec.hwName,
                  method: t(`cmp.engine_${rec.engine}`),
                  reason: t(`cmp.reason_${rec.reason}`),
                })}
              </span>
            </Glass>
          )}
          ...
          >
              ...
              {presets.custom.map((pr) => (
                <span
                  ...
                  onContextMenu={(e) =>
                    menu.open(e, [
                      {
                        label: t("cmp.deletePreset"),
                        icon: Trash2,
                        danger: true,
                        onClick: () => deletePreset(pr.name!),
                      },
                    ])
                  }
                >
                  <Badge tone="teal" active={presetActive(pr)} onClick={() => applyPreset(pr)}>
                    {pr.name}
                  </Badge>
                </span>
              ))}
              <Btn icon={Save} onClick={() => setSaveModal(true)}>
                {t("cmp.savePreset")}
              </Btn>
            </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  ...
                  <Field label={t("cmp.resolution")}>
                    <Select
                      value={p.targetHeight}
                      onChange={(e) => patch({ targetHeight: e.target.value })}
                      options={HEIGHTS.map((h) => ({
                        value: h,
                        label: h === "original" ? t("cmp.resOriginal") : `${h}p`,
                      }))}
                    />
                  </Field>
                  <Field label={t("cmp.audio")}>
                    <Select
                      ...
                      options={[
                        { value: "copy", label: t("cmp.audioCopy") },
                        { value: "aac", label: "AAC" },
                ...
                <Field label={t("cmp.method")}>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    <Badge
                      tone={p.engine === "auto" ? "amber" : "neutral"}
                      active={p.engine === "auto"}
                      onClick={() => patch({ engine: "auto" })}
                    >
                      {t("cmp.engineAuto")}
                    </Badge>
                    {engineOptions
                      ...
                      .map((e) => (
                        <Badge
                          key={e.id}
                          tone={e.kind === "gpu" ? "teal" : "violet"}
                          active={p.engine === e.id}
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <Field label={t("cmp.qualityMode")}>
                    <Select
                      value={p.qualityMode}
                      onChange={(e) => patch({ qualityMode: e.target.value })}
                      options={[
                  ...
                  {p.qualityMode === "constrained" && (
                    <Field label={t("cmp.maxKbps")}>
                      <input
                        ...
                        min={100}
                        step={100}
                        value={p.maxKbps || 4000}
                        onChange={(e) => patch({ maxKbps: Number(e.target.value) })}
                        style={{ width: 110 }}
                      />
                    </Field>
                  )}
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {crfOptimal && (p.qualityMode === "crf" || p.qualityMode === "constrained") && (
                    <Badge tone="teal">
                      {t("cmp.optimalCrf", { lo: crfOptimal[0], hi: crfOptimal[1] })}
                    </Badge>
                  )}
                  {bitrateOptimal && p.qualityMode === "bitrate" && (
                    <Badge tone="teal">
                      {t("cmp.optimalBitrate", { lo: bitrateOptimal[0], hi: bitrateOptimal[1] })}
                    </Badge>
                  )}
                  ...
                  {p.engine === "nvenc" && (
                    <Badge tone="neutral">{t("cmp.optimalSpeedNvenc")}</Badge>
                  )}
                </div>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={p.tenBit}
                      onChange={(e) => patch({ tenBit: e.target.checked })}
                    />
            ...
            {mode === "express" && (
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                <Badge tone="violet">
                  {p.engine === "auto" ? t("cmp.engineAuto") : t(`cmp.engine_${p.engine}`)}
                </Badge>
            ...
            {busy && job && (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span className="muted-sm">
                    {t(`cmp.stage_${job.stage}`)} ·{" "}
                    {t(`cmp.engine_${job.engineUsed || job.engine}`)}
                  </span>
                  <span className="muted-sm">
                ...
                {job.fallbacks?.length ? (
                  <span className="muted-sm">
                    {t("cmp.fallbacks", { list: job.fallbacks.join(", ") })}
                  </span>
                ) : null}
              </div>
            )}
            {!busy && (
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <Btn
                  ...
                  icon={Gauge}
                  onClick={start}
                  disabled={!file || (!!hw && !hw.ffmpeg.found)}
                >
                  {t("cmp.compress")}
                </Btn>
                <Btn icon={X} onClick={() => pick(null)} disabled={!file}>
                  {t("cmp.rechoose")}
                </Btn>
              </div>
            )}
...
function SplitCompare({
  ...
  const arRaw =
    fileMeta && fileMeta.w > 0 && fileMeta.h > 0
      ? fileMeta.w / fileMeta.h
  ...
  const hardSync = (from: HTMLVideoElement | null, to: HTMLVideoElement | null, tol = 0.25) => {
    if (from && to && Math.abs(from.currentTime - to.currentTime) > tol)
  ...
  const origBitrate =
    fileMeta && fileMeta.dur ? (job.size * 8) / fileMeta.dur : job.info?.bitRate || 0;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8, flex: 1, minHeight: 0 }}>
      ...
      <div className="cmp-player-wrap">
        <div
          ...
          style={{ ["--cmp-ar" as string]: ar, ["--cmp-max-h" as string]: "52vh" }}
        >
