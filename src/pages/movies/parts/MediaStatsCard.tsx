...
function BarList({
  ...
  return (
    <div className="mv-stat-block">
      <h4>{title}</h4>
      {items.length === 0 ? (
        <div className="muted-sm">—</div>
      ) : (
        <div className="mv-bars">
          {items.map((it) => (
            <div key={it.name} className="mv-bar-row">
              <span className="mv-bar-name" title={it.name}>
                {it.name}
              </span>
              <div className="mv-bar-track">
                <div
                  className={`mv-bar-fill tone-${tone}`}
    ...
    <div className="mv-stats">
      ...
      <div className="mv-stat-tiles">
        <Glass className="mv-tile">
          <Clock size={16} />
          <div>
            <b>{stats.totalHours}</b>
            <span>{t("movies.hoursWatched")}</span>
          </div>
        </Glass>
        <Glass className="mv-tile">
          <Check size={16} />
        ...
        <Glass className="mv-tile">
          ...
          <div>
            <b>{stats.avgRating || "—"}</b>
            <span>{t("movies.avgRating", { n: stats.ratingCount })}</span>
          </div>
        </Glass>
        <Glass className="mv-tile">
          <Film size={16} />
          <div>
            <b>{stats.totalTitles}</b>
            <span>{t("movies.titlesTracked")}</span>
          </div>
        </Glass>
      </div>
      ...
      <Glass className="mv-stats-block">
        <div className="mv-stat-status">
          <span>
            <i className="mv-dot tone-teal" />
            {t("movies.statusPlan")}: <b>{stats.watchlist.plan}</b>
          </span>
          <span>
            <i className="mv-dot tone-violet" />
            {t("movies.statusWatching")}: <b>{stats.watchlist.watching}</b>
          </span>
          <span>
            <i className="mv-dot tone-amber" />
            {t("movies.statusWatched")}: <b>{stats.watchlist.watched}</b>
          </span>
        </div>
      </Glass>
          ...
          <div className="mv-hist">
            {stats.ratingHistogram.map((h) => (
              <div key={h.value} className="mv-hist-col" title={`${h.value}: ${h.count}`}>
                <div
                  className="mv-hist-bar"
                  style={{ height: `${Math.round((h.count / maxHist) * 100)}%` }}
                />
                <span>{h.value}</span>
              </div>
            ))}
            ...
            <div className="mv-hist">
              {stats.monthly.map((m) => (
                <div key={m.month} className="mv-hist-col" title={`${m.month}: ${m.count}`}>
                  <div
                    className="mv-hist-bar tone-teal"
