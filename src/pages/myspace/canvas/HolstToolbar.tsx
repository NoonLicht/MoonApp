...
import type { CanvasTool, ShapeKind, ConnectorStyle, LineDash } from "@/pages/myspace/canvas/types";
...
function ShapeIcon({ kind }: { kind: ShapeKind }) {
  ...
  switch (kind) {
    case "rect":
      return (
        <svg width="15" height="15" viewBox="0 0 16 16">
          <rect x="2" y="3" width="12" height="10" {...s} />
        </svg>
      );
    case "rounded":
      return (
        <svg width="15" height="15" viewBox="0 0 16 16">
          <rect x="2" y="3" width="12" height="10" rx="3" {...s} />
        </svg>
      );
    case "circle":
      return (
    ...
    case "diamond":
      return (
        <svg width="15" height="15" viewBox="0 0 16 16">
          <polygon points="8,1.5 14.5,8 8,14.5 1.5,8" {...s} />
        </svg>
      );
    case "star":
      return (
        <svg width="15" height="15" viewBox="0 0 16 16">
          <polygon
            points="8,1.5 10,6 14.5,6 11,9 12,13.5 8,10.8 4,13.5 5,9 1.5,6 6,6"
    ...
    case "cloud":
      return (
        <svg width="15" height="15" viewBox="0 0 16 16">
          <path d="M4 12a3 3 0 1 1 .5-5.9A3.5 3.5 0 0 1 11.5 7 2.5 2.5 0 0 1 11 12Z" {...s} />
        </svg>
      );
  }
}
    ...
    <div className="holst-float holst-dock" onMouseDown={(e) => e.stopPropagation()}>
      ...
      {openPop === "line" && (
        <div className="holst-pop" style={{ left: 52, top: "30%" }}>
          ...
          <button
            className="holst-pop-item"
            onClick={() => p.onLineProps({ arrowStart: !p.lineProps.arrowStart })}
          >
            {p.lineProps.arrowStart ? "☑" : "☐"} Arrow start
          </button>
          <button
            className="holst-pop-item"
            onClick={() => p.onLineProps({ arrowEnd: !p.lineProps.arrowEnd })}
          >
            {p.lineProps.arrowEnd ? "☑" : "☐"} Arrow end
          </button>
          <button
            className="holst-pop-item"
            onClick={() => p.onLineProps({ animated: !p.lineProps.animated })}
          >
