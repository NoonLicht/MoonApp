...
/* ─── Theme / swatches ─── */
const THEME_COLORS = ["#f0a63d", "#8b7bf0", "#3fc7ab", "#ea6b6b", "#a9a8bb", "#f1f0f6"];
...
const HIGHLIGHTER_COLORS = ["#fbe46588", "#99e9b688", "#7fc3ed88", "#faa2c188", "#ffd8a888"];
const TRANSLUCENT_COLORS = [
  "rgba(240,166,61,0.25)",
  "rgba(139,123,240,0.25)",
  "rgba(63,199,171,0.25)",
  "rgba(234,107,107,0.25)",
  "rgba(241,240,246,0.15)",
  "rgba(169,168,187,0.25)",
];
  ...
  return (
    <div ref={ref} style={{ position: "relative", display: "inline-flex" }}>
      <button
        ...
        onMouseEnter={(e) => {
          if (!open) (e.currentTarget as HTMLButtonElement).style.background = "var(--track)";
        }}
        onMouseLeave={(e) => {
      ...
      >
        {mode === "text" ? (
          <Type size={14} strokeWidth={2} color="var(--text-tertiary)" />
        ) : (
          <Pen size={14} strokeWidth={2} color="var(--text-tertiary)" />
        )}
        <div
          style={{ width: 16, height: 3, borderRadius: 2, background: currentColor, marginTop: 1 }}
        />
      </button>
      {open && (
        <div
            ...
            {presetSwatches.map((c) => (
              <button
                ...
                style={{
                  ...
                  background: c,
                  border:
                    currentColor === c ? "2px solid var(--amber)" : "1px solid var(--glass-border)",
                  cursor: "pointer",
                  padding: 0,
                }}
              />
...
export default function EditingToolbar({
  ...
  const savedSel = useRef<{ start: number; end: number; text: string }>({
    start: 0,
    end: 0,
    text: "",
  });
  ...
  const fmt = useCallback(
    (type: string, value?: string) => {
      if (type === "textColor" || type === "bgColor" || type === "highlight")
        onFormat(type, value, savedSel.current);
  ...
  return (
    <div style={containerStyle}>
      ...
      <ColorPickerDropdown
        open={openDropdown === "highlight"}
        onToggle={() => toggleDropdown("highlight")}
        onClose={closeDropdown}
        currentColor={bgColor}
        onChange={(c) => {
          setBgColor(c || "rgba(240,166,61,0.25)");
          fmt("highlight", c);
        }}
        mode="bg"
      />
      <DropdownMenu
        open={openDropdown === "paste"}
        onClose={closeDropdown}
        trigger={
          <DropdownBtn
            icon={ClipboardPaste}
      ...
      >
        ...
        <DropdownItemBtn
          ...
          value="pastePlain"
          onClick={() => fmt("pastePlain")}
        />
        <DropdownItemBtn
          icon={WrapText}
          label="Duplicate"
          value="duplicate"
          onClick={() => fmt("duplicate")}
        />
      </DropdownMenu>
      <Sep />
      ...
      <TBtn
        icon={CheckSquare}
        title="Task List (- [ ])"
        active={activeFormats.has("task")}
        onClick={() => fmt("task")}
        disabled={isDisabled}
      />
      <DropdownMenu
        open={openDropdown === "comment"}
        onClose={closeDropdown}
        trigger={
          <DropdownBtn
            icon={Quote}
            open={openDropdown === "comment"}
            onClick={() => toggleDropdown("comment")}
            title="Blockquote / Comment"
          />
        }
      >
        <DropdownItemBtn
          ...
          label="Blockquote (> )"
          value="blockquote"
          onClick={() => fmt("blockquote")}
        />
        <DropdownItemBtn
          icon={MessageSquare}
          label="Note Callout (> [!NOTE])"
          value="callout"
          onClick={() => fmt("callout")}
        />
      </DropdownMenu>
      ...
      >
        ...
        <DropdownItemBtn
          ...
          value="inlineCode"
          onClick={() => fmt("inlineCode")}
        />
        <DropdownItemBtn
          icon={Braces}
          label="Code Block (```)"
          value="codeBlock"
          onClick={() => fmt("codeBlock")}
        />
        <DropdownItemBtn
          icon={Link}
          label="WikiLink ([[...]])"
          value="wikiLink"
          onClick={() => fmt("wikiLink")}
        />
        <DropdownItemBtn
          icon={FileText}
      ...
      >
        ...
        <DropdownItemBtn
          ...
          label="Align Left"
          value="alignLeft"
          onClick={() => fmt("alignLeft")}
        />
        <DropdownItemBtn
          icon={AlignCenter}
          label="Align Center"
          value="alignCenter"
          onClick={() => fmt("alignCenter")}
        />
        <DropdownItemBtn
          icon={AlignRight}
          label="Align Right"
          value="alignRight"
          onClick={() => fmt("alignRight")}
        />
      </DropdownMenu>
      <Sep />
      ...
      {/* ── Text Color ── */}
      <ColorPickerDropdown
        open={openDropdown === "textColor"}
        onToggle={() => toggleDropdown("textColor")}
        onClose={closeDropdown}
        currentColor={textColor}
        onChange={(c) => {
          setTextColor(c || "#f0a63d");
          fmt("textColor", c);
        }}
        mode="text"
      />
