...
export default function VideoPage() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      try {
        const stj = await api.getVideoInstall();
        setIns(stj);
      } catch {
        /* */
      }
  ...
  const fi = async () => {
    ...
    try {
      ...
      const pick =
        dh === "best"
          ? d.heights[0] || null
          : d.heights.find((x: number) => x <= Number(dh)) ||
            d.heights[d.heights.length - 1] ||
            null;
      setI(d);
      setH(pick);
  ...
  const si = async () => {
    try {
      const s = await api.startVideoInstall();
      setIns(s);
    } catch (e) {
      setErr((e as Error).message);
    }
  ...
  return (
    <div className="page">
          ...
          {ins.state === "working" ? (
            <div className="install-progress" style={{ width: "100%" }}>
              <div className="muted-sm">
                ...
                {ins.phase === "extract"
                  ? t("conv.installing")
                  : t("conv.downloading", { p: ins.progress })}
              </div>
              <ProgressBar value={ins.progress} />
            </div>
          ) : (
            <Btn variant="primary" icon={Terminal} onClick={si} style={{ width: 180 }}>
              {t("conv.install")}
            </Btn>
          )}
        </Glass>
      ...
      >
        ...
        />
        <Btn variant="primary" onClick={fi} disabled={st === "parsing"}>
          {st === "parsing" ? t("video.fetching") : t("video.fetch")}
        </Btn>
      </Glass>
      ...
      {st === "ready" && i && (
        <Glass className="media-preview">
          ...
          <div className="media-info">
            ...
            <div className="muted-sm">
              {i.durationString || ""} · {i.heights.length}res{" "}
              {i.subtitles && Object.keys(i.subtitles).length ? "· subs" : ""}
            </div>
            <div className="quality-row">
              {i.heights.slice(0, 15).map((_h, idx) => (
                <Badge key={idx} tone="amber" mono active={_h === h} onClick={() => setH(_h)}>
                  {_h >= 2160 ? "4K" : _h + "p"}
                </Badge>
              ))}
            </div>
            <div className="quality-row" style={{ gap: 4 }}>
              ...
              {Object.keys(i.subtitles || {}).length > 0 && (
                <Badge
                  ...
                  mono
                  active={sub.length > 0}
                  onClick={() => setSub(sub.length ? [] : Object.keys(i.subtitles))}
                >
                  <Subtitles size={12} />
                  Sub
                </Badge>
            ...
            <Btn variant="primary" icon={Download} onClick={dl} style={{ width: 260 }}>
              {t("video.download", { quality: h ? h + "p" : "best" })}
            </Btn>
          </div>
        </Glass>
      )}
