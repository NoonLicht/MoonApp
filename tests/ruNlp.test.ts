...
describe("server/ruNlp — ударения в омографах", () => {
  // Знак ударения — комбинирующая акута ПОСЛЕ гласной (U+0301): в исходнике она
  // не видна, поэтому в проверках пишем её явно.
  const A = "\u0301";

  it("вариант ударения выбирается по контексту вокруг слова", () => {
    expect(nlp.markStress("замок открыт")).toBe(`замо${A}к открыт`);
    expect(nlp.markStress("висячий замок")).toBe(`висячий за${A}мок`);
    expect(nlp.markStress("пшеничная мука")).toBe(`пшеничная мука${A}`);
    expect(nlp.markStress("мука")).toBe(`му${A}ка`);
    expect(nlp.markStress("белок")).toBe(`бело${A}к`);
  });

  it("соседние слова не теряются", () => {
    // Раньше шаблон захватывал «\\S*» по бокам слова и возвращал только само
    // слово: «открыт замок» превратилось бы в «замок» — слово «открыт» пропало.
    // Здесь же видно и работу контекста: «открыт» рядом → замо́к (про запирающее).
    expect(nlp.markStress("открыт замок")).toBe(`открыт замо${A}к`);
    // «мука из пшеницы» — продукт: смотрим контекст и после слова тоже.
    expect(nlp.markStress("Мука из пшеницы")).toBe(`Мука${A} из пшеницы`);
  });

  it("регистр слова сохраняется: «Замок» → «Замо́к», «ЗАМОК» → «ЗАМО́К»", () => {
    expect(nlp.markStress("Замок открыт")).toBe(`Замо${A}к открыт`);
    expect(nlp.markStress("ЗАМОК открыт")).toBe(`ЗАМО${A}К открыт`);
  });

  it("часть другого слова омографом не считается", () => {
    expect(nlp.markStress("замки висят")).toBe("замки висят");
    expect(nlp.markStress("белокрылый")).toBe("белокрылый");
  });
});
...
describe("server/ruNlp — нормализация", () => {
  ...
  it("ударения включаются явно и работают", () => {
    expect(nlp.normalize("замок")).toBe("замок");
    expect(nlp.normalize("замок", { markStress: true })).toBe("за\u0301мок");
    expect(nlp.normalize("замок открыт", { markStress: true })).toBe("замо\u0301к открыт");
  });
});
